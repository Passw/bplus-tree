...
int bp_get_previous(bp_db_t *tree, const bp_value_t *value, bp_value_t *previous)
{
    if (value->_prev_offset == 0 && value->_prev_length == 0) {
        return BP_ENOTFOUND;
    }
    return bp__value_load(tree, value->_prev_offset, value->_prev_length, previous);
}

int bp_update(
    bp_db_t *tree, const bp_key_t *key, const bp_value_t *value, bp_update_cb update_cb, void *arg)
{
    int ret;
                   ...
                   void *arg)
{
    ...
    ret = bp__page_bulk_insert(tree, tree->head.page, NULL, &left, &keys_iter, &values_iter, update_cb, arg);
    if (ret == BP_OK) {
        ret = bp__tree_write_head((bp__writer_t *) tree, NULL);
    }
...
int bp_bulk_set(bp_db_t *tree, const uint64_t count, const bp_key_t **keys, const bp_value_t **values)
{
    return bp_bulk_update(tree, count, keys, values, NULL, NULL);
}

int bp_removev(bp_db_t *tree, const bp_key_t *key, bp_remove_cb remove_cb, void *arg)
{
    int ret;
...
int bp_compact(bp_db_t *tree)
{
    ...
    ret = bp__writer_compact_finalize((bp__writer_t *) tree, (bp__writer_t *) &compacted);
    pthread_rwlock_unlock(&tree->rwlock);
...
int bp_get_range(bp_db_t *tree, const bp_key_t *start, const bp_key_t *end, bp_range_cb cb, void *arg)
{
...
int bp_updates(bp_db_t *tree, const char *key, const char *value, bp_update_cb update_cb, void *arg)
{
    bp_key_t bkey;
    bp_value_t bvalue;
                    ...
                    void *arg)
{
    ...
    ret = bp_bulk_update(
        tree, count, (const bp_key_t **) &bkeys, (const bp_value_t **) &bvalues, update_cb, arg);
...
int bp_bulk_sets(bp_db_t *tree, const uint64_t count, const char **keys, const char **values)
{
    return bp_bulk_updates(tree, count, keys, values, NULL, NULL);
}

int bp_removevs(bp_db_t *tree, const char *key, bp_remove_cb remove_cb, void *arg)
{
    bp_key_t bkey;
...
int bp_get_filtered_ranges(
    bp_db_t *tree, const char *start, const char *end, bp_filter_cb filter, bp_range_cb cb, void *arg)
{
    bp_key_t bstart;
    bp_key_t bend;
...
int bp_get_ranges(bp_db_t *tree, const char *start, const char *end, bp_range_cb cb, void *arg)
{
    return bp_get_filtered_ranges(tree, start, end, bp__default_filter_cb, cb, arg);
}
